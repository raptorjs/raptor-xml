...
function getSchema(el, nodeType, parentSchema) {
...
function _expected(parentSchema, isAttribute) {
    var expected = [];
    //console.log(curType);
...
function Reader(schema, options) {
    if (!options) {
        options = {};
    }
    this.skipping = false;
...
Reader.prototype = {
    ...
    _setProperty: function(el, schema, targetObject, value, context) {
        ...
        if (typeof schema === 'function') {
            schema(targetObject, value, context);
            return;
        }
        ...
        if (typeof value === 'string') {
            ...
            if (schema._type === BOOLEAN) {
                if (value.length === 0) {
                    // empty value provided so let's not try to interpret this
                    value = undefined;
                } else {
                    /*
    ...
    read: function(xmlSrc, filePath) {
        ...
        var handleCharacters = function(text) {
            ...
            if (!context) {
                if (strings.trim(text)) {
                    _this.error('Unexpected text: ' + text);
                }
                else {
                    return;
                }
        ...
        saxParser.on({
            ...
            startElement: function (el) {
                ...
                if (parentContext && parentSchema && parentContext.text) {

                    var trimmed = strings.trim(parentContext.text);

                    if (parentSchema._type === OBJECT) {
                        if (parentSchema._text) {
                            _this._setProperty("text", parentSchema._text, parentContext.object, parentContext.text, parentContext);
                        }
                        else if (parentSchema._setText) {
                            parentSchema._setText(parentContext.object, _this._parseProp(parentContext.text, parentContext));
                        }
                        else if (trimmed) {
                            _this.error("Unexpected text: " + parentContext.text);
                        }
                    }
                    else if (trimmed) {
                        _this.error("Unexpected text: " + parentContext.text);
                ...
                if (!curSchema) {
                    var missingName = el.getNamespaceURI() ? el.getNamespaceURI() + ':' + el.getLocalName() : el.getLocalName();
                    _this.error("Unexpected element: <" + missingName + ">. Expected one of: " + _expected(parentSchema));
                }
                ...
                if (!_this.skipping) {
                    ...
                    for (var i=0, len=attrs.length, attr; i<len; i++) {
                        ...
                        var attrContext = extend({}, context);
                ...
                if (_this.skipping !== true) {
                    ...
                    if (curSchema._type === STRING || curSchema._type === BOOLEAN) {
                        ...
                        if (context.text != null) {
                            var trimmed = strings.trim(context.text);
                            if (curSchema._text) {
